...
static hook::cdecl_stub<hook::FlexStruct*(const uint32_t modelHash, uint32_t* modelId)> fwArchetypeManager_GetArchetypeFromHashKey([]()
{
	return hook::get_call(hook::get_pattern("E8 ? ? ? ? 0F B7 45 ? 66 89 45 ? 8B 45 ? 48 8D 4D ? 0B C7"));
});
...
static bool CVehicleCreationDataNode__CanApply(hook::FlexStruct* thisptr, rage::netObject* netObj /*Hidden argument*/)
{
	...
	hook::FlexStruct* modelInfo = fwArchetypeManager_GetArchetypeFromHashKey(hash, nullptr);
	if (!modelInfo)
	{
		return false;
	}
